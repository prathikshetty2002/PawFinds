...
const PetCards = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/approving/${props.pet._id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: "Approved"
        }),
  ...
  const deleteFormsAdoptedPet = async () => {
    setIsDeleting(true)
    try {
      const deleteResponses = await fetch(`${API_BASE_URL}/form/delete/many/${props.pet._id}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/delete/${props.pet._id}`, {
        method: 'DELETE'
      })
    ...
    <div className='req-containter'>
      <div className='pet-view-card'>
        <div className='pet-card-pic'>
          <img src={`${API_BASE_URL}/images/${props.pet.filename}`} alt={props.pet.name} />
        </div>
