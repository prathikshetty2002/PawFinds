...
const FormCard = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/approving/${props.form.petId}`, {
        method: 'PUT',
        body: JSON.stringify({
  ...
  const deleteFormAdoptedPet = async () => {
    try {
      const deleteResponse = await fetch(`${API_BASE_URL}/form/delete/many/${props.form.petId}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    setIsDeleting(true)
    try {
      const response = await fetch(`${API_BASE_URL}/form/reject/${props.form._id}`, {
        method: 'DELETE'
      })
